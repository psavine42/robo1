...
def balls_setup(frame, positions):
    """
    Place a list of balls in a reference frame.
    The reference object (ball) must have been previously copied to the clipboard.
    """
    nballs = len(positions)
    step = 1.0/(nballs - 1)
    for i in range(nballs):
        newball = frame.Paste()
        newball.setName('ball ' + str(i))
        newball.setPose(transl(positions[i]))
        newball.setVisible(True, False)
        newball.Recolor([1-step*i, step*i, 0.2, 1])
...
def TCP_On(toolitem, tcp_id):
    ...
    furthermore, it will output appropriate function calls
     on the generated robot program (call to TCP_On)"""
...
def TCP_Off(toolitem, tcp_id, itemleave=0):
    ...
    furthermore, it will output appropriate function calls on the generated
    robot program (call to TCP_Off)"""
...
idLeave = 0
idTCP = 0
target_app_frame = transl(2*BALL_DIAMETER, 2*BALL_DIAMETER, 4*BALL_DIAMETER)*\
                   roty(pi)*transl(0, 0, -APPROACH)
...
while idTake >= 0:
    ...
    for i in range(ntake):
        ...
        # calculate target wrt frame1:
        # rotation about Y is needed since Z and X axis are inverted
